...
def build(mcu, imu, hex, clk, sw0, vccGpio, lp, sleep):
    ...
    subprocess.run(f"e: && cd {root_path}/firmware/~/ && {root_path}/firmware/~/.venv/Scripts/activate.bat && west build --board {boardBuild}  --pristine=always SlimeVR-Tracker-nRF-{iteration} --build-dir SlimeVR-Tracker-nRF-{iteration}/build  --  -DNCS_TOOLCHAIN_VERSION=NONE  -DBOARD_ROOT=../SlimeVR-Tracker-nRF-{iteration}", shell=True)
...
def fetch_prebuild():
    try:
        ...
        subprocess.run(f"e: && cd {root_path}/uploads/ && wget -O firmware{iteration}.uf2 {firmware_url}", shell=True)
